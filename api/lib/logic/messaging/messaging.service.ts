...
export class MessagingService {
  ...
  private escapeMarkdown(text: string): string {
    ...
    return text
      .replace(/([_*\[\]`])/g, '\\$1'); // Only escape critical markdown characters
  }
  ...
  private async formatMorningMessage(
    ...
    this.logger.debug('Formatting morning message with calendar events and tasks');
    ...
    if (calendarEvents && calendarEvents.length > 0) {
      this.logger.debug(`Adding ${calendarEvents.length} calendar events to message`);
    ...
    if (tasks && tasks.length > 0) {
      ...
      for (const [groupName, groupTasks] of Object.entries(groupedTasks)) {
        ...
        if (groupName !== 'Ungrouped') {
          message += `\n*${groupName}*\n`;
          this.logger.debug(`Adding task group: ${groupName} with ${groupTasks.length} tasks`);
        }
  ...
  async formatAfternoonMessage(
    ...
    const groupedRemainingTasks = await this.groupingService.groupTasks(remainingTasks);
    
    // Generate a motivational message based on completed tasks
    const motivationMessage = await this.generateMotivationMessage(completedTasks);
    ...
    if (completedTasks.length === 0) {
      ...
      const sortedCompletedTasks = completedTasks.sort((a, b) => b.priority - a.priority);
   ...
   */
  async formatEveningMessage(completedTasks: Task[], inboxTasks: Task[]): Promise<string> {
    ...
    if (inboxTasks.length === 0) {
      ...
      // Group inbox tasks
      const groupedInboxTasks = await this.groupingService.groupTasks(inboxTasks);
    ...
    const eveningReflection = await this.generateEveningReflection(completedTasks, inboxTasks);
  ...
  async sendMorningMessage(): Promise<void> {
    try {
      ...
      this.logger.debug(`Generating morning message for ${currentDate.toISOString()}`);
      ...
      this.logger.log(`Found ${calendarEvents.length} calendar events for today`);
      
      // Log each calendar event to help with debugging
      calendarEvents.forEach(event => {
        this.logger.debug(`Calendar event: "${event.summary}" at ${event.startTime.toISOString()} - ${event.endTime.toISOString()}`);
      });
      ...
      dueTasks.forEach(task => {
        this.logger.debug(`Task: "${task.content}" with priority ${task.priority} due: ${task.dueDate?.toISOString() || 'No date'}`);
      });
  ...
  async sendAfternoonMessage(): Promise<void> {
    ...
    try {
      ...
      const completedTasks = await this.todoistService.getCompletedTasksSince(lastMorningDate);
      ...
      const remainingTasks = await this.todoistService.getRemainingTasksForAfternoon();
      
      this.logger.debug(`📊 AFTERNOON API FILTER RESULT: ${remainingTasks.length} remaining tasks (due today/overdue, excluding inbox)`);
  ...
  async sendEveningMessage(): Promise<void> {
    try {
      ...
      this.logger.debug(`Fetching completed tasks since: ${today.toISOString()}`);
      const completedTasks = await this.todoistService.getCompletedTasksSince(today);
      this.logger.log(`Found ${completedTasks.length} completed tasks today`);
      
      // Log each completed task to help with debugging
      completedTasks.forEach(task => {
        this.logger.debug(`Completed task: "${task.content}" on ${task.completedAt?.toISOString() || 'unknown'}`);
      });

      // Fetch inbox tasks - ensure we're only getting actual inbox tasks without due dates
      const inboxTasks = await this.todoistService.getInboxTasksWithoutDueDates();
      this.logger.log(`Found ${inboxTasks.length} inbox tasks requiring triage`);
      ...
      if (inboxTasks.length === 0 && completedTasks.length === 0) {
        this.logger.log('No tasks to report in evening message - inbox is empty and no tasks completed');
      }

      // Format the evening message with completed tasks and inbox tasks
      const message = await this.formatEveningMessage(completedTasks, inboxTasks);
   ...
   * @private
   */
  private async generateMotivationMessage(completedTasks: Task[]): Promise<string> {
    if (completedTasks.length === 0) {
    ...
    try {
      ...
      this.logger.error(`Failed to generate motivation message: ${error.message}`);
   ...
   * @private
   */
  private async generateEveningReflection(completedTasks: Task[], inboxTasks: Task[]): Promise<string> {
    try {
      ...
      const inboxContext = inboxTasks.length > 0
      ...
      // Extract the response
      const content = response.choices[0].message.content;
      return content?.trim() || "Today's work is done - rest well and tomorrow brings new opportunities.";
    } catch (error) {
      this.logger.error(`Failed to generate evening reflection: ${error.message}`);
   ...
   * @private
   */
  private async generateMorningInspiration(dueTasks: Task[], calendarEvents: CalendarEvent[]): Promise<string> {
    try {
      // Create context from today's tasks and calendar
      const tasksContext = dueTasks.length > 0 
        ? `Today's tasks: ${dueTasks.map(task => task.content).slice(0, 5).join(', ')}${dueTasks.length > 5 ? '...' : ''}`
      ...
      const highPriorityCount = dueTasks.filter(task => task.priority >= 3).length;
      const priorityContext = highPriorityCount > 0 
      ...
      const response = await this.openaiClient.chat.completions.create({
        ...
        messages: [
          { role: 'system', content: systemPrompt },
          { role: 'user', content: `${tasksContext}. ${calendarContext}${priorityContext}.` },
        ],
        max_tokens: 60,
      });
      ...
      // Extract the response
      const content = response.choices[0].message.content;
      return content?.trim() || "Today is full of possibilities - approach each task with intention and focus.";
    } catch (error) {
      this.logger.error(`Failed to generate morning inspiration: ${error.message}`);
