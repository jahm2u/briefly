...
export class TodoistAdapter {
  ...
  constructor(apiTokenOrInstance: string | TodoistApi | any) {
    if (typeof apiTokenOrInstance === 'string') {
      ...
      if (typeof apiTokenOrInstance.getTasks === 'function' || 
          typeof apiTokenOrInstance.getTasksByFilter === 'function') {
        // Use the mock object directly
        this.api = apiTokenOrInstance;
      } else {
        throw new Error('Invalid mock object: must implement getTasks or getTasksByFilter method');
      }
    } else {
      throw new Error('Invalid argument: must provide either API token string or TodoistApi instance');
    }
  }
  ...
  async getTasks(options?: Record<string, any>): Promise<Task[]> {
    ...
    if (options && options.filter) {
      const response = await this.api.getTasksByFilter({ query: options.filter, limit: options.limit || 200 });
