...
async function testRecurringEvents() {
  ...
  try {
    ...
    const recurringEvents = vevents.filter((vevent) =>
    ...
    console.log(
      `Start: ${rangeStart.toISOString()} = ${rangeStart.toLocaleString('en-US', { timeZone: 'America/Sao_Paulo' })}`,
    );
    console.log(
      `End: ${rangeEnd.toISOString()} = ${rangeEnd.toLocaleString('en-US', { timeZone: 'America/Sao_Paulo' })}`,
    );
    console.log('');
      ...
      if (dtstart) {
        try {
          ...
          while (occurrence && count < 10) {
            ...
            console.log(
              `  Occurrence ${count + 1}: ${occurrence.toString()} = ${jsDate.toLocaleString('en-US', { timeZone: 'America/Sao_Paulo' })}`,
            );

            // Check if in range
            const inRange =
              occurrence.compare(icalRangeStart) >= 0 &&
              occurrence.compare(icalRangeEnd) <= 0;
